...
int main()
{
    ...
    auto last_emit = std::chrono::high_resolution_clock::now();
    while (app.run())
    {
        ...
        if (sf::Mouse::isButtonPressed(sf::Mouse::Left))
        {
            ...
            if (now - last_emit > std::chrono::milliseconds(300))
            {
                last_emit = now;
                emit = true;
                position = app.getWorldMousePosition();
            }
        }
        else
        {
        ...
        if (solver.objects.size() < 20000 && emit)
        {
            ...
            for (uint32_t i{10}; i--;)
            {
                ...
                position.x = position.x + 1.0f * i;
                const auto id = solver.createObject(position);
                // solver.objects[id].last_position.x -= 0.2f;
                solver.objects[id].last_position.y -= 0.1f;
                solver.objects[id].acceleration.x = 0.0f;
                solver.objects[id].acceleration.y = 0.0f;
                solver.objects[id].color = ColorUtils::getRainbow(id * 0.1f);
                /*if (colors.empty())
                {
